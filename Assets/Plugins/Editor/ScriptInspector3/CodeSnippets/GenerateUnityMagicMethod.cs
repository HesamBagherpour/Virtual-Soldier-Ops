...
class GenerateUnityMagicMethod : ISnippetProvider
{
	...
	class MagicMethod<T> : SnippetCompletion, IMagicMethod
	{
		...
		public override string Expand()
		{
			...
			string modifiers;
			switch (SISettings.magicMethods_accessModifier)
			{
			case "<none>":
				modifiers = "";
				break;
			...
			case "public":
				...
				break;
			default:
				modifiers = contextType.IsSealed ? "" : "protected ";
				break;
			}
	...
	private static List<SnippetCompletion> monoBehaviourMagicMethods = new List<SnippetCompletion> {
		...
		new MagicMethod<MonoBehaviour>("void OnJointBreak2D(Joint2D brokenJoint)"),
		...
		new MagicMethod<MonoBehaviour>("void OnParticleSystemStopped()"),
		new MagicMethod<MonoBehaviour>("void OnParticleTrigger()"),
		new MagicMethod<MonoBehaviour>("void OnParticleUpdateJobScheduled()"),
		// Mouse
		...
		new MagicMethod<MonoBehaviour>("void OnApplicationFocus(bool hasFocus)"),
		new MagicMethod<MonoBehaviour>("IEnumerator OnApplicationFocus(bool hasFocus)"),
		new MagicMethod<MonoBehaviour>("void OnApplicationPause(bool pauseStatus)"),
		new MagicMethod<MonoBehaviour>("IEnumerator OnApplicationPause(bool pauseStatus)"),
		...
		new MagicMethod<MonoBehaviour>("IEnumerator OnPreCull()"),
		...
		new MagicMethod<MonoBehaviour>("void OnFailedToConnectToMasterServer(NetworkConnectionError error)"),
	...
	private static List<SnippetCompletion> editorWindowMagicMethods = new List<SnippetCompletion> {
		new MagicMethod<EditorWindow>("void CreateGUI()"),
		new MagicMethod<EditorWindow>("void OnBecameInvisible()"),
		new MagicMethod<EditorWindow>("void OnBecameVisible()"),
		...
		// Inherited
		new MagicMethod<ScriptableObject>("void Awake()"),
		new MagicMethod<ScriptableObject>("void OnDisable()"),
		new MagicMethod<ScriptableObject>("void OnEnable()"),
		new MagicMethod<ScriptableObject>("void OnValidate()"),
		new MagicMethod<ScriptableObject>("void Reset()"),
	};
	...
	private static List<SnippetCompletion> editorMagicMethods = new List<SnippetCompletion> {
		new MagicMethod<Editor>("bool HasFrameBounds()"),
		new MagicMethod<Editor>("Bounds OnGetFrameBounds()"),
	...
	private static List<SnippetCompletion> assetPostprocessorMagicMethods = new List<SnippetCompletion> {
		...
		new MagicMethod<AssetPostprocessor>("static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths, bool didDomainReload)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessAnimation(GameObject root, AnimationClip clip)"),
		...
		new MagicMethod<AssetPostprocessor>("void OnPostprocessCubemap(Cubemap texture)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessGameObjectWithAnimatedUserProperties(GameObject gameObject, EditorCurveBinding[] bindings)"),
		...
		new MagicMethod<AssetPostprocessor>("void OnPostprocessMaterial(Material material)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessMeshHierarchy(GameObject root)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessModel(GameObject root)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessPrefab(GameObject root)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessSpeedTree(GameObject root)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessSprites(Texture2D texture, Sprite[] sprites)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessTexture(Texture2D texture)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessTexture2DArray(Texture2DArray texture)"),
		new MagicMethod<AssetPostprocessor>("void OnPostprocessTexture3D(Texture3D texture)"),
		new MagicMethod<AssetPostprocessor>("void OnPreprocessAnimation()"),
		new MagicMethod<AssetPostprocessor>("void OnPreprocessAsset()"),
		new MagicMethod<AssetPostprocessor>("void OnPreprocessAudio()"),
		new MagicMethod<AssetPostprocessor>("void OnPreprocessCameraDescription(CameraDescription description, Camera camera, AnimationClip[] animations)"),
		new MagicMethod<AssetPostprocessor>("void OnPreprocessLightDescription(LightDescription description, Light light, AnimationClip[] animations)"),
		new MagicMethod<AssetPostprocessor>("void OnPreprocessMaterialDescription(MaterialDescription description, Material material, AnimationClip[] animations)"),
	...
	private static TypeDefinitionBase monoBehaviourType =
		TypeReference.To(typeof(MonoBehaviour)).definition as TypeDefinitionBase;
	private static TypeDefinitionBase editorWindowType =
		TypeReference.To(typeof(EditorWindow)).definition as TypeDefinitionBase;
	private static TypeDefinitionBase scriptableWizardType =
		TypeReference.To(typeof(ScriptableWizard)).definition as TypeDefinitionBase;
	private static TypeDefinitionBase scriptableObjectType =
		TypeReference.To(typeof(ScriptableObject)).definition as TypeDefinitionBase;
	private static TypeDefinitionBase editorType =
		TypeReference.To(typeof(Editor)).definition as TypeDefinitionBase;
	private static TypeDefinitionBase assetPostprocessorType =
		TypeReference.To(typeof(AssetPostprocessor)).definition as TypeDefinitionBase;
		...
		Scope scope)
	{
		...
		foreach (var magic in magicMethods)
		{
			...
			tempLeaf.token.SetText(magic.name);
