...
static class GenericMenuExtensions
{
		...
		GenericMenu.MenuFunction func)
	{
		...
		if (Application.platform == RuntimePlatform.OSXEditor)
			...
			menu.AddItem(new GUIContent(winText + " _" + winKey.ToUpperInvariant()), on, func);
#else
		if (Application.platform == RuntimePlatform.OSXEditor)
...
#endif
{
	...
	private static void OnProjectItemGUI(string item, Rect selectionRect)
	{
		...
		if (path.EndsWithDll() || path.EndsWithExe() || path.EndsWithIgnoreCase(".shadergraph"))
		{
			return;
		}
	...
	public static bool OnOpenAsset(int instanceID, int line)
	{
		...
		{
			var assetPath = AssetDatabase.GetAssetPath(instanceID);
			if (assetPath.EndsWithIgnoreCase(".shadergraph"))
			{
				return false;
			}
		}
		...
		{
			if (asset is MonoScript || asset is TextAsset || asset is Shader || asset is UnityEngine.UIElements.StyleSheet)
			{
				var assetPath = AssetDatabase.GetAssetPath(instanceID);
		...
		if (!SISettings.dontOpenAssets && !SISettings.handleOpenAssets)
		{
			...
			bool isText = !isScript && (asset is TextAsset || asset is UnityEngine.UIElements.StyleSheet);
	...
	startAgain:
		...
		{
			if (codeWindow && codeWindow.textEditor.targetGuid.Equals(guid, System.StringComparison.OrdinalIgnoreCase))
			{
				codeWindow.Show();
				codeWindow.Focus();
				if (characterIndex < 0)
		...
		if (target == null)
		{
			...
			if (target == null)
			{
				...
				if (target == null)
				{
					target = AssetDatabase.LoadAssetAtPath(path, typeof(UnityEngine.UIElements.StyleSheet)) as UnityEngine.UIElements.StyleSheet;
					if (target == null)
						return null;
				}
			}
	...
	public static FGCodeWindow OpenNewWindow(Object target, FGCodeWindow nextTo, bool reuseExisting)
	{
		...
		if (reuseExisting || target == null)
		{
			...
			if (target == null)
				target = Selection.activeObject as UnityEngine.UIElements.StyleSheet;
			if (target == null)
			{
				target = Selection.activeObject as DefaultAsset;
				if (target != null && !FGDefaultAssetInspector.IsTextFile(target))
					target = null;
			}
			if (target == null)
				return null;
	...
	private void OnEnable()
	{
		...
		if (targetAsset == null)
			targetAsset = Selection.activeObject as UnityEngine.UIElements.StyleSheet;
		...
		try
		{
			...
			if (targetAsset != null)
			{
				...
				if (assetPath.StartsWithIgnoreCase("assets/"))
				{
					...
					titleContent.tooltip = assetPath == "" ? null : titleContent.text + " \nin " + assetPath;
				}
#endif
			}
			else
	...
	private void OnFirstRepaint()
	{
		...
		textEditor.OnEnable(targetAsset, this, false);
		...
		foreach (FGCodeWindow wnd in codeWindows)
		{
			if (wnd && wnd.targetAssetGuid == guid)
			{
				...
				if (assetPath.StartsWithIgnoreCase("assets/"))
				{
					...
					wnd.titleContent.tooltip = assetPath == "" ? null : wnd.titleContent.text + " \nin " + assetPath;
				}
#endif
					...
					foreach (Object obj in DragAndDrop.objectReferences)
					{
						...
						if (obj is GameObject)
						{
							...
							foreach (MonoBehaviour mb in monoBehaviours)
							{
								...
								if (monoScript != null)
								{
									...
									if (!string.IsNullOrEmpty(assetPath) &&
										(assetPath.StartsWithIgnoreCase("assets/") || assetPath.StartsWithIgnoreCase("packages/")) &&
										!assetPath.EndsWithDll() && !assetPath.EndsWithExe())
									{
						...
						if (assetPath.StartsWithIgnoreCase("assets/") || assetPath.StartsWithIgnoreCase("packages/"))
						{
							if (obj is MonoScript)
								accepted.Add(obj);
							else if (obj is TextAsset || obj is Shader || obj is UnityEngine.UIElements.StyleSheet)
								accepted.Add(obj);
							...
							{
								if (FGDefaultAssetInspector.IsTextFile(obj))
									accepted.Add(obj);
							}
						}
					}
