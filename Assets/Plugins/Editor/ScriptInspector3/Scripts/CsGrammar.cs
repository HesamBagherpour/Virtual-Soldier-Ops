...
public class CsGrammar : FGGrammar
{
	...
	public override short TokenToId(CharSpan tokenText)
	{
		short id = parser.TokenToId(tokenText);
		return id < 0 ? tokenIdentifier : id;
	}
	...
	void InitializeTokenCategories()
	{
		...
		tokenPropertyBody = TokenToId(".PROPERTYBODY");
	...
	//	NAME.lookahead.Add(new TokenSet(tokenName));
		...
		var globalAttributes = new Id("globalAttributes");
		...
		var x = Id.scanner;
		//var flatExpression = new Id("flatExpression");
		...
		var tupleType = new Id("tupleType");
		var tupleElement = new Id("tupleElement");
		...
		var keyValueInitializer = new Id("keyValueInitializer");
		var dictionaryInitializerList = new Id("dictionaryInitializerList");
		...
		parser.Add(new Rule("PARTIAL",
			new Ifs("partial", IDENTIFIER) | "partial"
	...
	//	parser = new Parser(r_compilationUnit, this);
		...
		if (!CsParser.isCSharp4)
		{
			parser.Add(new Rule("classMemberDeclaration",
				new Seq(
					...
					modifiers,
					constantDeclaration
					| new Ifs("void", "void" - IDENTIFIER, "void" - methodDeclaration)
					| new Ifs("async", ASYNC - modifiers - new Opt(PARTIAL) - "void", ASYNC - modifiers - new Opt(PARTIAL) - "void" - methodDeclaration)
					| new Ifs("async", ASYNC - modifiers - IDENTIFIER, ASYNC - modifiers - type - methodDeclaration)
					| new Ifs("partial", PARTIAL - ("void" - methodDeclaration | classDeclaration | structDeclaration | interfaceDeclaration))
		...
		parser.Add(new Rule("WHERE",
			new Ifs("where", IDENTIFIER) | "where"
			) { contextualKeyword = true });
		...
		{
			var UNMANAGED = new Id("UNMANAGED");
			parser.Add(new Rule(UNMANAGED,
				new Ifs("unmanaged", IDENTIFIER) | "unmanaged"
				) { contextualKeyword = true });
			
			parser.Add(new Rule("typeParameterConstraintList",
				(new Lit("class") | "struct") - new Opt("," - secondaryConstraintList)
				| new Ifs("unmanaged", UNMANAGED - new Opt("," - secondaryConstraintList))
				| secondaryConstraintList
			));
		}

		parser.Add(new Rule("secondaryConstraintList",
			secondaryConstraint - new Many(new IfNot(new Lit(",") - "new", "," - secondaryConstraint)) - new Opt("," -constructorConstraint)
...
//	| 'struct' ;
	...
	//	var declarationStatement = new Id("declarationStatement");
		...
		var fixedStatement = new Id("fixedStatement");
		...
		var unsafeStatement = new Id("unsafeStatement");
		var localVariableDeclaration = new Id("localVariableDeclaration");
		var localFunctionDeclaration = new Id("localFunctionDeclaration");
		var localFunctionBody = new Id("localFunctionBody");
		var expressionBodiedLocalFunction = new Id("expressionBodiedLocalFunction");
		//var localVariableOrFunctionDeclaration = new Id("localVariableOrFunctionDeclaration");
		...
		var isVariableDeclaration = new Id("isVariableDeclaration");
		var isVariableDeclarator = new Id("isVariableDeclarator");

		var VAR = new Id("VAR");
		
		var DISCARD = new Id("DISCARD");
		...
		var implicitDeconstructDeclaration = new Id("implicitDeconstructDeclaration");
		var implicitDeconstructList = new Id("implicitDeconstructList");
		var implicitDeconstructTarget = new Id("implicitDeconstructTarget");
		var implicitDeconstructVariableDeclarator = new Id("implicitDeconstructVariableDeclarator");
		
		var explicitDeconstructDeclaration = new Id("explicitDeconstructDeclaration");
		var explicitDeconstructList = new Id("explicitDeconstructList");
		var explicitDeconstructTarget = new Id("explicitDeconstructTarget");
		var explicitDeconstructVariableDeclaration = new Id("explicitDeconstructVariableDeclaration");
		var explicitDeconstructVariableDeclarator = new Id("explicitDeconstructVariableDeclarator");
		...
		{
			parser.Add(new Rule("DISCARD",
				IDENTIFIER | "_"
				) { contextualKeyword = true });

			parser.Add(new Rule("statement",
				new Ifs("await", IsAwaitInsideAsyncMethod, awaitExpression)
				| new If((type | "var") - IDENTIFIER - (new Lit(";") | "=" | "[" | ","), localVariableDeclaration - ";")
				| new If(IDENTIFIER - ":", labeledStatement)
				| new Ifs("var", IDENTIFIER - "(", implicitDeconstructDeclaration - ";")
				| new Ifs("(", explicitDeconstructList - "=", explicitDeconstructDeclaration - ";")
				| new If(attributes - modifiers - (new Ifs("void", "void" - IDENTIFIER, "void") | type) - IDENTIFIER - (new Lit("(") | "<"), localFunctionDeclaration)
				| new If("using" - (IDENTIFIER | "var"), "using" - localVariableDeclaration - ";")
				| embeddedStatement
			...
			parser.Add(new Rule(expressionBodiedLocalFunction,
				"=>" - (throwExpression | expression) - ";"
				) { semantics = SemanticFlags.LocalFunctionBodyScope });

			parser.Add(new Rule(localFunctionDeclaration,
				attributes - modifiers - (new Ifs("void", "void" - IDENTIFIER, "void") | type) - NAME - new Opt(typeParameterList) - "(" - new Opt(formalParameterList) - ")" - new Opt(typeParameterConstraintsClauses) - (localFunctionBody | expressionBodiedLocalFunction)
				) { semantics = SemanticFlags.LocalFunctionDeclarator });
			
			parser.Add(new Rule(implicitDeconstructDeclaration,
				VAR - implicitDeconstructList - "=" - expression
				));
			
			parser.Add(new Rule(implicitDeconstructList,
				"(" - implicitDeconstructTarget - "," - implicitDeconstructTarget
				- new Many("," - implicitDeconstructTarget) - ")"
				));
			
			parser.Add(new Rule(implicitDeconstructTarget,
				new Ifs("_", DISCARD)
				| implicitDeconstructVariableDeclarator | implicitDeconstructList
				));
			
			parser.Add(new Rule(implicitDeconstructVariableDeclarator,
				NAME
				) { semantics = SemanticFlags.TupleDeconstructVariableDeclarator });
			
			parser.Add(new Rule(explicitDeconstructDeclaration,
				explicitDeconstructList - "=" - expression
				));
			
			parser.Add(new Rule(explicitDeconstructList,
				"(" - explicitDeconstructTarget - "," - explicitDeconstructTarget
				- new Many("," - explicitDeconstructTarget) - ")"
				));
			
			parser.Add(new Rule(explicitDeconstructTarget,
				new Ifs("_", IDENTIFIER - (new Lit(",") | ")"), DISCARD)
				| new Ifs("(", explicitDeconstructList)
				| explicitDeconstructVariableDeclaration // | "_"
				));
			
			parser.Add(new Rule(explicitDeconstructVariableDeclaration,
				localVariableType - (
					new Ifs("_", DISCARD)
					| explicitDeconstructVariableDeclarator
				)
				));
			
			parser.Add(new Rule(explicitDeconstructVariableDeclarator,
				NAME
				) { semantics = SemanticFlags.TupleDeconstructVariableDeclarator });
		}
	...
	//		));
		...
		parser.Add(new Rule("embeddedStatement",
			...
			| new If(
				s => {
					if (s.Current.text != "yield")
						return false;
					var next = s.Lookahead().text;
					return next == "return" || next == "break";
				}, yieldStatement)
			| new If(new Lit("using") - "(", usingStatement)
			| fixedStatement
			...
			| lockStatement
			| unsafeStatement
			| "yield return" | "yield break;" // auto-completion hints only
			| ".STATEMENT"
			));
		...
		parser.Add(new Rule(unsafeStatement,
			new Seq( "unsafe", block )
		));
		...
		parser.Add(new Rule("fixedStatement",
			new Seq( "fixed", "(", localVariableDeclaration, ")", embeddedStatement )
		) { semantics = SemanticFlags.FixedStatementScope });
		...
		else
		{
			parser.Add(new Rule("foreachStatement",
				new Lit("foreach") - "(" - (
					new Ifs("(", explicitDeconstructList - "in", explicitDeconstructList) |
					new Ifs("var", IDENTIFIER - "(", VAR - implicitDeconstructList) |
					localVariableType - NAME
				) - "in" - expression - ")" - embeddedStatement
			) { semantics = SemanticFlags.ForStatementScope | SemanticFlags.ForEachVariableDeclaration });
		}
		...
		parser.Add(new Rule("accessorModifiers",
			"readonly" - new Opt("internal" - new Opt("protected")
				| "protected" - new Opt(new Lit("private") | "internal")
				| "public"
				| new Lit("private") - new Opt("protected"))
			| ("internal" - new Opt("protected")
				| "protected" - new Opt(new Lit("private") | "internal")
				| "public"
				| new Lit("private") - new Opt("protected")) - new Opt("readonly")
		));
		...
		parser.Add(new Rule("accessorDeclarations",
			attributes - new Opt(accessorModifiers) - (
				new Ifs("get", getAccessorDeclaration - new Opt(attributes - new Opt(accessorModifiers) - setAccessorDeclaration))
		...
		parser.Add(new Rule("eventAccessorDeclarations",
			attributes - (
			new Ifs("add", addAccessorDeclaration - attributes - removeAccessorDeclaration)
		...
		else
		{
			parser.Add(new Rule("modifiers",
				new Many(
					new Lit("new") | "public" | "protected" | "internal" | "private" | "abstract"
					| "sealed" | "static" | "readonly" | "volatile" | "virtual" | "override" | "extern" | "unsafe"
				)
			));
		}
		...
		parser.Add(new Rule("operatorDeclarator",
			new Seq( "operator",
				new If((new Lit("+") | "-") - "(" - type - IDENTIFIER - ",", overloadableBinaryOperator - "(" - operatorParameter - binaryOperatorPart - ")")
				| new If(overloadableUnaryOperator, overloadableUnaryOperator - "(" - operatorParameter - ")")
		...
		parser.Add(new Rule("overloadableUnaryOperator",
			new Ifs("+", new Lit("+")) | new Ifs("-", new Lit("-")) | "!" |  "~" |  "++" | "--" | "true" |  "false"
			));

		parser.Add(new Rule("overloadableBinaryOperator",
			new Ifs("+", new Lit("+")) | new Ifs("-", new Lit("-")) | 
			// >> check needed
			"*" | "/" | "%" | "&" | "|" | "^" | "<<" | new If(new Seq(">",">"), new Seq(">",">")) | "==" | "!=" | ">" | "<" | ">=" | "<="
			));
		...
		if (!CsParser.isCSharp4)
		{
			parser.Add(new Rule("structMemberDeclaration",
				new Seq(
					...
					constantDeclaration
					| new Ifs("void", "void" - IDENTIFIER, "void" - methodDeclaration)
					| new Ifs("async", ASYNC - modifiers - PARTIAL - "void", ASYNC - modifiers - PARTIAL - "void" - methodDeclaration)
					| new Ifs("async", ASYNC - modifiers - "void", ASYNC - modifiers - "void" - methodDeclaration)
					| new Ifs("async", ASYNC - modifiers - PARTIAL - type - memberName - "(", ASYNC - modifiers - PARTIAL - type - methodDeclaration)
					| new Ifs("async", ASYNC - modifiers - type - memberName - "(", ASYNC - modifiers - type - methodDeclaration)
					| new Ifs("partial", PARTIAL, PARTIAL - ("void" - methodDeclaration | classDeclaration | structDeclaration | interfaceDeclaration))
					| new If(IDENTIFIER - "(", constructorDeclaration)
					| "ref" - (
						new If(PARTIAL, PARTIAL - structDeclaration)
						| structDeclaration
		...
		var interfaceEventDeclarators = new Id("interfaceEventDeclarators");
		var interfaceEventDeclarator = new Id("interfaceEventDeclarator");
			...
			parser.Add(new Rule("interfaceMemberDeclaration",
				new Seq( attributes, modifiers,
					...
					| new Opt("ref" - new Opt("readonly")) - type - (
						new If(memberName - "(", methodDeclaration)
						| new If(memberName - (new Lit("{") | "=>"), interfacePropertyDeclaration)
						| interfaceIndexerDeclaration )
					| interfaceEventDeclaration
					| structDeclaration | classDeclaration | enumDeclaration | delegateDeclaration | interfaceDeclaration
					| ".INTERFACEBODY" )
				));
		}
		else if (!CsParser.isCSharp4)
		{
			parser.Add(new Rule("interfaceMemberDeclaration",
				new Seq( attributes, modifiers,
					new Ifs("void", "void" - IDENTIFIER, "void" - interfaceMethodDeclaration)
					| new Ifs("async", ASYNC - modifiers - "void", ASYNC - modifiers - "void" - interfaceMethodDeclaration)
					| new Ifs("async", ASYNC - modifiers - type - memberName - "(", ASYNC - modifiers - type - interfaceMethodDeclaration)
					| new Opt("ref" - new Opt("readonly")) - type - (
		...
		if (CsParser.isCSharp9)
		{
			parser.Add(new Rule("interfacePropertyDeclaration",
				NAME - ("{" - interfaceAccessorDeclarations - "}" | "=>" - expression - ";")
		...
		parser.Add(new Rule("interfaceEventDeclaration",
			"event" - typeName - interfaceEventDeclarators - ";"
			));

		parser.Add(new Rule("interfaceEventDeclarators",
			interfaceEventDeclarator - new Many("," - interfaceEventDeclarator)
			));

		parser.Add(new Rule("interfaceEventDeclarator",
			NAME
			) { semantics = SemanticFlags.InterfaceEventDeclarator });
		...
		parser.Add(new Rule("interfaceAccessorDeclarations",
			new Seq( attributes,
				new Ifs("get", interfaceGetAccessorDeclaration - new Opt(attributes - interfaceSetAccessorDeclaration))
		...
		{
			parser.Add(new Rule("delegateDeclaration",
				"delegate" - (new Ifs("void", "void" - IDENTIFIER, "void") | new Opt("ref" - new Opt("readonly")) - type) - NAME - new Opt(typeParameterList)
					- "(" - new Opt(formalParameterList) - ")" - new Opt(typeParameterConstraintsClauses) - ";"
				) { semantics = SemanticFlags.DelegateDeclaration | SemanticFlags.TypeDeclarationScope });
		}
		
		var globalAttributeTargetSpecifier = new Id("globalAttributeTargetSpecifier");
		...
		parser.Add(new Rule("globalAttributeTargetSpecifier",
			ATTRIBUTETARGET - ":"
			));
		...
		parser.Add(new Rule("globalAttributes",
			new Ifs("[", s => { var next = s.Lookahead().text; return next == "assembly" || next == "module"; },
			"[" - globalAttributeTargetSpecifier - attribute - new Many(new Ifs(",", s => s.Lookahead().tokenId == tokenIdentifier, "," - attribute)) - new Opt(",") - "]")
		...
		parser.Add(new Rule("attributes",
			new Many(
				...
				new Many(new Ifs(",", s => s.Lookahead().tokenId == tokenIdentifier, "," - attribute)) - new Opt(",") - "]"
				)
			) { semantics = SemanticFlags.AttributesScope });
		...
		var basicType = new Id("basicType");
		//var pointerType = new Id("pointerType");

		parser.Add(new Rule(basicType,
			new Lit("bool") | "byte" | "char" | "decimal" | "double" | "float" | "int" | "long"
			| "void" | "sbyte" | "short" | "string" | "uint" | "ulong" | "ushort"
			));
		
		//parser.Add(new Rule("pointerType",
		//	(basicType | typeName) - "*" - new Many("*") - new Opt(rankSpecifiers)
		//	));
		
		parser.Add(new Rule("type",
			new If((basicType | typeName) - "*", (basicType | typeName) - new Many("*") - new Opt(rankSpecifiers))
			| (predefinedType | typeName) - new Opt("?") - new Many(rankSpecifiers)
			));
		...
		if (SymbolDefinition.supportsDynamicType)
		{
	...
	//		));
		...
		if (CsParser.isCSharp4)
		{
			parser.Add(new Rule("namespaceOrTypeName",
				new If(IDENTIFIER - "::", globalNamespace - "::") - typeOrGeneric - new Many("." - typeOrGeneric)
				));
		}
		else
		{
			parser.Add(new Rule("tupleType",
				"(" - tupleElement - "," - tupleElement - new Many("," - tupleElement) - ")"
				));
			
			parser.Add(new Rule("tupleElement",
				type - new Opt(NAME)
				));
			
			parser.Add(new Rule("namespaceOrTypeName",
				new If("(" - tupleElement - ",", tupleType) |
				new If(IDENTIFIER - "::", globalNamespace - "::") - typeOrGeneric - new Many("." - typeOrGeneric)
				));
		}
...
//				| IDENTIFIER
		...
		//var decompositionExpression = new Id("decompositionExpression");
		//var decompositionExpressionList = new Id("decompositionExpressionList");
		
		//var decompositionDeclaration = new Id("decompositionDeclaration");
		//var decompositionDeclarationList = new Id("decompositionDeclarationList");
		...
		{
			parser.Add(new Rule("expression",
				/*new If("(" - variableReference - ",", decompositionExpression)
				|*/ new If(unaryExpression - assignmentOperator, assignment)
				| nonAssignmentExpression
				));
		...
		//var keywordAfterCastExpression = new Id("keywordAfterCastExpression");
		//parser.Add(new Rule("keywordAfterCastExpression",
		//	new Lit("base") | "bool" | "byte" | "case" | "char" | "checked" | "decimal" | "default" | "delegate"
		//	| "double" | "false" | "fixed" | "float" | "int" | "lock" | "long" | "new" | "null" | "object"
		//	| "ref" | "sbyte" | "short" | "sizeof" | "stackalloc" | "string" | "switch" | "this" | "throw"
		//	| "true" | "typeof" | "uint" | "ulong" | "unchecked" | "unsafe" | "ushort" | "using" | "void"
		//	| "async" | "await"));
		...
		if (CsParser.isCSharp4)
		{
			parser.Add(new Rule("unaryExpression",
				...
				new If("(" - (
					predefinedType - (new Opt("?") | new Many("*")) - new Opt(rankSpecifiers) - ")"
					| new Seq("void", "*")
					| typeName - (
						(rankSpecifiers | new Lit("?") - new Opt(rankSpecifiers) | "*" - new Many("*") - new Opt(rankSpecifiers)) - ")"
						| ")" - (new Lit("~") | "!" | "(" | IDENTIFIER | LITERAL
							| "base" | "bool" | "byte" | "case" | "char" | "checked" | "decimal" | "default" | "delegate"
							| "double" | "false" | "fixed" | "float" | "int" | "lock" | "long" | "new" | "null" | "object"
							| "ref" | "sbyte" | "short" | "sizeof" /*| "stackalloc"*/ | "string" | "switch" | "this" | "throw"
							| "true" | "typeof" | "uint" | "ulong" | "unchecked" | "unsafe" | "ushort" | "using" | "void")
						)
		...
		else
		{
			...
			parser.Add(new Rule(unaryExpression,
				...
				| new Ifs("await", IsAwaitInsideAsyncMethod, awaitExpression)
				| primaryExpression - new Many(new Lit("++") | "--") // TODO: Fix this! Post increment operators should be primaryExpressionPart
				| new Seq( new Lit("+") | "-" | "!" | "~" | "^" | "&" | "*", unaryExpression | ".EXPECTEDTYPE" )
				| preIncrementExpression
				| preDecrementExpression
				));
		}
		...
		if (CsParser.isCSharp8)
		{
			parser.Add(new Rule("assignmentOperator",
				new Lit("=") | "+=" | "-=" | "*=" | "/=" | "%=" | "&=" | "|=" | "^=" | "<<=" | new Seq(">", ">=") | new Seq("??", "=")
				));
		}
		else
		{
		...
		//var tupleExpression = new Id("tupleExpression");
		var tupleExpressionElement = new Id("tupleExpressionElement");
		...
		if (!CsParser.isCSharp4)
		{
			parser.Add(new Rule("NAMEOF",
				new Ifs("nameof", IDENTIFIER | "nameof")
			){ contextualKeyword = true });
...
//*
		...
		else
		{
			parser.Add(new Rule("primaryExpression",
				"null"
				| new Ifs("async", IDENTIFIER - "delegate", ASYNC - anonymousMethodExpression)
				| primaryExpressionStart - new Many(new If((new Opt("?") - (new Lit(".") | "[" | "->")) | "(" | "!", primaryExpressionPart))
				| new Seq( "new",
					((nonArrayType | ".EXPECTEDTYPE") - (objectCreationExpression | arrayCreationExpression))
					| implicitArrayCreationExpression
					| anonymousObjectCreationExpression,
					new Many(new If((new Opt("?") - (new Lit(".") | "[")) | "(" | "!" | "->", primaryExpressionPart)))
				| anonymousMethodExpression
				));
		}

		var parenOrTupleExpression = new Id("parenOrTupleExpression");
		...
		parser.Add(new Rule("typeofExpression",
			new Lit("typeof") - "(" - (new Ifs("void", new Lit("void") - ")", "void") | type) - ")"
			));
		...
		if (CsParser.isCSharp4)
		{
			parser.Add(new Rule("parenOrTupleExpression",
				"(" - expression - ")"
				));
		...
		else
		{
			parser.Add(new Rule("interpolatedStringLiteral",
				INTERP_STR_WHOLE
				| INTERP_STR_START - new Opt(stringInterpolation)
				- new Many(new Opt(INTERP_STR_MID) - new Opt(stringInterpolation)) - INTERP_STR_END
				));
			...
			parser.Add(new Rule("tupleExpressionElement",
				new If(IDENTIFIER - ":", NAME - ":" - expression)
				| expression
				));
			
			//parser.Add(new Rule("tupleExpression",
			//	"(" - tupleExpressionElement - new Many("," - tupleExpressionElement) - ")"
			//	));
			...
			parser.Add(new Rule("primaryExpressionStart",
				...
				| new Ifs("nameof", IDENTIFIER - "(", nameofExpression)
				| new If(IDENTIFIER - typeArgumentList /*- (new Lit("(") | ")" | ":" | ";" | "," | "." | "?" | "==" | "!="*/, IDENTIFIER - typeArgumentList)
				| new If(IDENTIFIER - "::", globalNamespace - "::") - IDENTIFIER
				//| new If("(" - IDENTIFIER - ":", tupleExpression)
				//| new If("(" - expression - ",", tupleExpression)
				| parenOrTupleExpression
				| "this"
				| "base"
				| typeofExpression
		...
		{
			var nullForgivingOperator = new Id("nullForgivingOperator");
			
			parser.Add(new Rule(nullForgivingOperator,
				"!"
			));
			
			parser.Add(new Rule("primaryExpressionPart",
				new Opt("?") - (accessIdentifier | brackets)
				| arguments
		...
		parser.Add(new Rule("accessIdentifier",
			"." - (IDENTIFIER - new If(typeArgumentList, typeArgumentList) | "." - new Opt(expression))
		...
		else
		{
			parser.Add(new Rule("outVariableDeclaration",
				localVariableType - (new Ifs("_", DISCARD) | outVariableDeclarator)
				));
		...
		else
		{
			parser.Add(new Rule("collectionInitializer",
				elementInitializerList - new Opt(",")
				| dictionaryInitializerList - new Opt(",")
				));

			parser.Add(new Rule("dictionaryInitializerList",
				keyValueInitializer - new Many(new IfNot(new Opt(",") - "}", "," - keyValueInitializer))
				));

			parser.Add(new Rule("keyValueInitializer",
		...
		parser.Add(new Rule("objectInitializer",
			new Opt(memberInitializerList) - new Opt("," - new Opt(".MEMBERINITIALIZER"))
			));
		...
		var INTO = new Id("INTO"); parser.Add(new Rule("INTO", new Ifs("into", IDENTIFIER | "into")) { contextualKeyword = true });
		var ORDERBY = new Id("ORDERBY"); parser.Add(new Rule("ORDERBY", new Ifs("orderby", IDENTIFIER | "orderby")) { contextualKeyword = true });
		var JOIN = new Id("JOIN"); parser.Add(new Rule("JOIN", new Ifs("join", IDENTIFIER | "join")) { contextualKeyword = true });
		var LET = new Id("LET"); parser.Add(new Rule("LET", new Ifs("let", IDENTIFIER | "let")) { contextualKeyword = true });
		var ON = new Id("ON"); parser.Add(new Rule("ON", new Ifs("on", IDENTIFIER | "on")) { contextualKeyword = true });
		var EQUALS = new Id("EQUALS"); parser.Add(new Rule("EQUALS", new Ifs("equals", IDENTIFIER | "equals")) { contextualKeyword = true });
		var BY = new Id("BY"); parser.Add(new Rule("BY", new Ifs("by", IDENTIFIER | "by")) { contextualKeyword = true });
		...
		parser.Add(new Rule("nonAssignmentExpression",
			...
			| new Ifs("async", IDENTIFIER - anonymousFunctionSignature - "=>", ASYNC - lambdaExpression)
			| new Ifs("from", IDENTIFIER - IDENTIFIER - "in", queryExpression)
			| new Ifs("from", IDENTIFIER - new Opt(type) - IDENTIFIER - "in", queryExpression)
		...
		parser.Add(new Rule("anonymousFunctionSignature",
			new If(new Lit("(") - ")", new Lit("(") - ")")
			| new Seq(
				"(",
				new If(IDENTIFIER - (new Lit(",") | ")"), implicitAnonymousFunctionParameterList)
		...
		parser.Add(new Rule(localFunctionBody,
			"{" - statementList - "}"
			) { semantics = SemanticFlags.LocalFunctionBodyScope });
		...
		parser.Add(new Rule("queryBody",
			...
			) - (new Ifs("select", selectClause) | "select" | groupClause) - new Opt(queryContinuation)
		...
		parser.Add(new Rule("queryBodyClause",
			new Ifs("from", fromClause)
			| new Ifs("let", letClause)
			| new Ifs("join", joinClause)
			| new Ifs("orderby", orderbyClause)
			| whereClause
			));
		...
		parser.Add(new Rule("ordering",
			expression
			- new If(s => s.Current.text == "ascending" || s.Current.text == "descending", ASCENDING_OR_DESCENDING)
			));
		...
		{
			//parser.Add(new Rule(nullCoalescingExpression,
			//	flatExpression - new Many("??" - (throwExpression | flatExpression))
			//	) { autoExclude = true });
		}
		...
		//	flatExpression - new Many( (new Lit("==") | "!=") - (flatExpression | ".EXPECTEDTYPE") )
		//	) { autoExclude = true });
		...
		else
		{
			if (CsParser.isCSharp9)
			{
				//parser.Add(new Rule(isVariableDeclaration,
				//	localVariableType - new If(s => s.Current.text != "and" && s.Current.text != "or", isVariableDeclarator)
				//	));
			}
			else
			{
				parser.Add(new Rule(isVariableDeclaration,
					localVariableType - new Opt(isVariableDeclarator)
					));
			}

			parser.Add(new Rule(isVariableDeclarator,
				NAME
				) { semantics = SemanticFlags.IsVariableDeclarator });

			//parser.Add(new Rule(relationalExpression,
			//	shiftExpression - new Many(
			//		new Seq( new Lit("<") | ">" | "<=" | ">=", (shiftExpression | ".EXPECTEDTYPE") )
			//		| ("as" - (new If(type2 - "?" - expression - ":", type2) | type))
			//		| ("is" - (new If(type2 - "?" - expression - ":", type2) | new If(isVariableDeclaration, isVariableDeclaration) | shiftExpression))
			//	)
			//	) { autoExclude = true });
		}
		...
		var switchExpressionBody = new Id("switchExpressionBody");
		var switchExpressionArm = new Id("switchExpressionArm");

		parser.Add(new Rule(switchExpressionBody,
			"{" - new Opt(switchExpressionArm - new Many(new IfNot(new Lit(",") - "}", "," - switchExpressionArm)) - new Opt(",")) - "}"
			) { semantics = SemanticFlags.SwitchBlockScope });

		parser.Add(new Rule(switchExpressionArm,
			(
				new Ifs("_", DISCARD)
				| (
					(new If(constantExpression - (WHEN | "=>"), constantExpression) | caseVariableDeclaration)
					- new Opt(WHEN - expression)
				)
			)
		...
		if (CsParser.isCSharp9)
		{
			...
			parser.Add(new Rule("NOT",
				new Ifs("not", IDENTIFIER) | "not"
				) { contextualKeyword = true });

			parser.Add(new Rule("AND",
				new Ifs("and", IDENTIFIER)
				) { contextualKeyword = true });

			parser.Add(new Rule("OR",
				new Ifs("or", IDENTIFIER)
				) { contextualKeyword = true });

			var pattern = new Id("pattern");
			//var relationalPattern = new Id("relationalPattern");
			
			//parser.Add(new Rule("relationalPattern",
			//	(new Lit("<") | ">" | "<=" | ">=") - unaryExpression
			//	));
				
			parser.Add(new Rule("pattern",
				new Many(new Ifs("not", NOT)) -
				(
					(new Lit("<") | ">" | "<=" | ">=") - unaryExpression
					//| new If(isVariableDeclaration, isVariableDeclaration)
					| unaryExpression - new If(s => s.Current.text != "and" && s.Current.text != "or", isVariableDeclarator))
				- new Many(new Ifs2("or", "and", (new Ifs("or", OR) | AND) - pattern))
				));
			//parser.Add(new Rule("pattern",
			//	(new Ifs("not", NOT - unaryExpression) | new Opt(new Lit("<") | ">" | "<=" | ">=") - unaryExpression)
			//	- (new Many(new Ifs2("or", "and", (new Ifs("or", OR) | AND) - (new Ifs("not", NOT - unaryExpression) | new Opt(new Lit("<") | ">" | "<=" | ">=") - unaryExpression))))
			//	));

			parser.Add(new Rule(flatExpression,
				new Seq( unaryExpression, new Many(
					new Ifs(">", new Seq(">", ">"), new Seq(">", ">", unaryExpression))
					| new Ifs("??", new Seq(new Lit("??"), unaryExpression | throwExpression))
					| new Seq(new Lit("*") | "/" | "%" | "+" | "-" | "<<" | "&&" | "||", unaryExpression)
					| new Seq(new Lit("==") | "!=" | "<" | ">" | "<=" | ">=" | "|" | "&" | "^", unaryExpression | ".EXPECTEDTYPE")
					| ("as" - (new If(type2 - "?" - expression - ":", type2) | type))
					| ("is" - (new If(type2 - "?" - expression - ":", type2)
						//| new If(new Ifs("not", NOT) - type - (new Ifs("or", IDENTIFIER) | "and"), pattern)
						//| new If(isVariableDeclaration, isVariableDeclaration)
						| pattern
						//| (new Ifs("not", NOT - unaryExpression) | new Opt(new Lit("<") | ">" | "<=" | ">=") - unaryExpression) - (new Many(new If(new Ifs("or", OR) | new Ifs("and", AND), (new Ifs("or", OR) | AND) - (new Ifs("not", NOT - unaryExpression) | new Opt(new Lit("<") | ">" | "<=" | ">=") - unaryExpression))))
						))
					| new Seq(new Lit("switch"), switchExpressionBody)
					)
					/*| ".." - new Opt(expression)*/ )
				| new Lit(".") - "." - new Opt(expression)
				));
		}
		else
		{
			parser.Add(new Rule(flatExpression,
				new Seq( unaryExpression, new Many(
					new If(new Seq(">", ">"), new Seq(">", ">", unaryExpression))
					| new Seq(new Lit("??"), unaryExpression | throwExpression)
					| new Seq(new Lit("*") | "/" | "%" | "+" | "-" | "<<" | "&&" | "||", unaryExpression)
					| new Seq(new Lit("==") | "!=" | "<" | ">" | "<=" | ">=" | "|" | "&" | "^", unaryExpression | ".EXPECTEDTYPE")
					| ("as" - (new If(type2 - "?" - expression - ":", type2) | type))
					| ("is" - (new If(type2 - "?" - expression - ":", type2) | new If(isVariableDeclaration, isVariableDeclaration) | unaryExpression))
					| new Seq(new Lit("switch"), switchExpressionBody)
					)
					/*| ".." - new Opt(expression)*/ )
				| new Lit(".") - "." - new Opt(expression)
	...
	public void Parse(List<FGTextBuffer.FormattedLine> lines, string bufferName)
	{
	//	var s = new Stopwatch();
	//	s.Start();
		...
		{
			Debug.LogError("Parsing exception at line: " + (scanner.CurrentLine() + 1) + ", token " + scanner.CurrentTokenIndex() + " with:\n    " + e + " at " + e.StackTrace);
	...
	public bool ParseLine(Scanner scanner, int line)
	{
		...
		while (scanner.CurrentLine() <= line)
		{
	...
	public static ParseTree.Node EnclosingScopeNode(ParseTree.Node node, SemanticFlags scopeType1, SemanticFlags scopeType2, SemanticFlags scopeType3, SemanticFlags scopeType4)
	{
		while (node != null)
		{
			var scopeSemantics = node.semantics & SemanticFlags.ScopesMask;
			...
			{
				if (scopeType1 == scopeSemantics || scopeType2 == scopeSemantics ||
					scopeType3 == scopeSemantics || scopeType4 == scopeSemantics)
				{
					return node;
				}
			}
		...
		if (node.scope == null || nodeScopeAsSDS != null && nodeScopeAsSDS.declaration == null)
		{
			...
			switch (scopeSemantics)
			{
				...
				case SemanticFlags.LambdaExpressionBodyScope:
					...
					node.scope = new BodyScope(node) { parentScope = enclosingScope, definition = declaration.definition };
					break;
				case SemanticFlags.LocalFunctionBodyScope:
				...
				case SemanticFlags.FixedStatementScope:
					node.scope = new LocalScope(node) { parentScope = enclosingScope };
					break;
		...
		if (node.declaration == null)
		{
			...
			switch (declarationSemantics)
			{
				...
				case SemanticFlags.UsingNamespace:
					...
					//(enclosingScope as NamespaceScope).declaration.importedNamespaces[namespaceToImport] = new TypeReference(node.ChildAt(0));
					//return null;
				...
				case SemanticFlags.UsingAlias:
					node.declaration = new SymbolDeclaration { parseTreeNode = node, kind = SymbolKind.UsingAlias };
					break;
				...
				case SemanticFlags.ConversionOperatorDeclarator:
					...
					break;
				case SemanticFlags.LocalFunctionDeclarator:
					modifiersNode = node.parent.FindChildByName("modifiers");
				...
				case SemanticFlags.LocalVariableDeclarator:
					...
					break;
				case SemanticFlags.TupleDeconstructVariableDeclarator:
					node.declaration = new SymbolDeclaration { parseTreeNode = node, kind = SymbolKind.TupleDeconstructVariable };
					break;
				case SemanticFlags.OutVariableDeclarator:
					...
					break;
				case SemanticFlags.IsVariableDeclarator:
					enclosingScopeNode = null;
					node.declaration = new SymbolDeclaration { parseTreeNode = node, kind = SymbolKind.IsVariable };
					break;
				...
				case SemanticFlags.ForEachVariableDeclaration:
					enclosingScopeNode = null;
					enclosingScope = GetNodeScope(node, fileName);
				...
				case SemanticFlags.InterfacePropertyDeclaration:
					...
					node.declaration = new SymbolDeclaration { parseTreeNode = node, kind = SymbolKind.Property };
					break;
				case SemanticFlags.InterfaceEventDeclarator:
					node.declaration = new SymbolDeclaration { parseTreeNode = node, kind = SymbolKind.Event };
					break;
				...
				case SemanticFlags.AnonymousMethodDeclaration:
					enclosingScopeNode = EnclosingScopeNode(node.parent,
						...
						SemanticFlags.LocalFunctionBodyScope,
				...
				case SemanticFlags.None:
					...
					break;
				default:
					Debug.LogError("Unhandled case " + declarationSemantics + " for node " + node);
					break;
			}
			...
			if (node.declaration != null)
			{
				...
				{
					var saved = TypeReference.dontResolveNow;
					TypeReference.dontResolveNow = node.declaration.kind != SymbolKind.UsingAlias;
					try
					{
					...
					{
						TypeReference.dontResolveNow = saved;
					}
	...
	private static Modifiers ParseModifiers(ParseTree.BaseNode node)
	{
		...
		for (var i = 0; i < root.numValidNodes; ++i)
		{
			...
			if (mod == null)
				continue;
			var textSpan = mod.token.text;
			if (textSpan == "public")
				mods |= Modifiers.Public;
			else if (textSpan == "internal")
				mods |= Modifiers.Internal;
			else if (textSpan == "protected")
				mods |= Modifiers.Protected;
			else if (textSpan == "private")
				mods |= Modifiers.Private;
			else if (textSpan == "static")
				mods |= Modifiers.Static;
			else if (textSpan == "new")
				mods |= Modifiers.New;
			else if (textSpan == "sealed")
				mods |= Modifiers.Sealed;
			else if (textSpan == "abstract")
				mods |= Modifiers.Abstract;
			else if (textSpan == "readonly")
				mods |= Modifiers.ReadOnly;
			else if (textSpan == "volatile")
				mods |= Modifiers.Volatile;
			else if (textSpan == "virtual")
				mods |= Modifiers.Virtual;
			else if (textSpan == "override")
				mods |= Modifiers.Override;
			else if (textSpan == "extern")
				mods |= Modifiers.Extern;
			else if (textSpan == "ref")
				mods |= Modifiers.Ref;
			else if (textSpan == "out")
				mods |= Modifiers.Out;
			else if (textSpan == "this")
				mods |= Modifiers.This;
			else if (textSpan == "partial")
				mods |= Modifiers.Partial;
			else if (textSpan == "async")
	...
	private bool IsAwaitInsideAsyncMethod(IScanner s)
	{
		...
		ParseTree.Node methodDeclarationNode = s.CurrentParseTreeNode.parent;
		while (methodDeclarationNode != null)
		{
			var ruleName = methodDeclarationNode.RuleName;
			if (ruleName == "methodDeclaration")
				break;
			
			if (ruleName == "lambdaExpression")
			{
				var firstNode = methodDeclarationNode.parent.NodeAt(0);
				return firstNode != null && firstNode.RuleName == "ASYNC";
			}
			
			if (ruleName == "anonymousMethodExpression")
			{
				var firstNode = methodDeclarationNode.parent.NodeAt(0);
				return firstNode != null && firstNode.RuleName == "ASYNC";
			}
			
			methodDeclarationNode = methodDeclarationNode.parent;
		}
	...
	public class Scanner : IScanner
	{
		...
		public static Scanner New(CsGrammar grammar, List<FGTextBuffer.FormattedLine> formattedLines, string fileName)
		{
			if (pool.Count == 0)
				return new Scanner(grammar, formattedLines, fileName);
			
			var scanner = pool.Pop();
			scanner.grammar = grammar;
			scanner.lines = formattedLines;
		...
		protected Scanner(CsGrammar grammar, List<FGTextBuffer.FormattedLine> formattedLines, string fileName)
		{
			...
			if (EOF == null)
				EOF = new SyntaxToken(SyntaxToken.Kind.EOF, CharSpan.Empty) { tokenId = grammar.tokenEOF };
		}
		...
		{
			/*var result =*/ (CurrentGrammarNode ?? grammar.r_compilationUnit).CollectCompletions(tokenSet, this, grammar.tokenIdentifier);
			if (CurrentGrammarNode != null && tokenSet.Matches(grammar.tokenIdentifier))
			{
				var c = Current;
				Current = new SyntaxToken(SyntaxToken.Kind.Identifier, "special");
				Current.tokenId = grammar.tokenIdentifier;
		...
		public override void InsertMissingToken(ErrorMessageProvider errorMessage)
		{
			...
			var missingToken = new SyntaxToken(SyntaxToken.Kind.Missing, CharSpan.Empty) { formattedLine = missingLine };
			...
			{
				++currentTokenIndex;
				//Current = tokens != null ? tokens[currentTokenIndex] : EOF;
			}
		}
		...
		//	nonTriviaTokenIndex = -1;
			...
			Rule startRule = grammar.r_compilationUnit;
		...
		//	timeLookaheads = new Dictionary<Node, long>();
			...
			if (token == EOF)
			{
				...
				currentTokenIndex = -1;
				tokens = null;
				ErrorParseTreeNode = CurrentParseTreeNode = parseTreeNode;

				return false;
			}

			currentLine = token != null ? token.formattedLine.index : 0;
		...
		public override bool MoveNext()
		{
			...
			while (MoveNextSingle())
			{
				...
				if (tokenKind <= SyntaxToken.Kind.VerbatimStringLiteral)
					continue;
				...
				if (token.tokenId == -1)
				{
					switch (tokenKind)
					{
						...
						case SyntaxToken.Kind.InterpolatedStringFormatLiteral:
							...
							break;
						default:
							Debug.LogError("Unhandled case value in switch statement!");
							break;
					}
				}
		...
		{
			while (/*tokens == null ||*/ ++currentTokenIndex >= tokens.Count)
			{
				currentTokenIndex = -1;
				if (++currentLine >= lines.Count)
				{
