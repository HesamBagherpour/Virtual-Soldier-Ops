	...
	public static FGTooltip Create(FGTextEditor editor, Rect tokenRect, ParseTree.Leaf leaf, bool horizontal = false, bool showError = true)
	{
		...
		if (symbolDefinition != null)
		{
			try
			{
				...
				//if (leaf.parent.RuleName == "accessIdentifier")
				//{
				//	var asMethod = symbolDefinition as MethodDefinition;
				//	isExtensionMethod = asMethod != null && asMethod.IsExtensionMethod;
		...
		if (owner != null && UnityEditorInternal.InternalEditorUtility.isApplicationActive)
			owner.Focus();
#endif
