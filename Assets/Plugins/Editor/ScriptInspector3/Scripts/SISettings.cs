...
{
	public const string prefix = "FlipbookGames.ScriptInspector.";
...
public static class SISettings
{
	...
	public static IntOption lineSpacing = Create("LineSpacing", 2);
	public static IntOption wrappedLinesSpacing = Create("WrappedLinesSpacing", 0);
	...
	public static BoolOption codeFoldingCode = Create("CodeFoldingCode", true);
	public static BoolOption codeFoldingCodeInspector = Create("CodeFoldingCodeInspector", true);
	public static BoolOption autoFoldRegions = Create("AutoFoldRegions", true);
	...
	public static bool autoReloadAssemblies = true;
	...
	public static BoolOption swapShiftAndCtrlEnter = Create("SwapShiftAndCtrlEnter", false);
	...
	public static StringOption magicMethods_accessModifier = Create("MagicMethods.AccessModifier", "protected");
	...
	public static void CopyPreferences()
	{
		...
		sb.Append(codeFoldingCode.ToJson()); sb.Append(",\n");
		sb.Append(codeFoldingCodeInspector.ToJson()); sb.Append(",\n");
		...
		sb.Append(swapShiftAndCtrlEnter.ToJson()); sb.Append(",\n");
		...
		sb.Append(magicMethods_accessModifier.ToJson()); sb.Append(",\n");
	...
	static SISettings()
	{
		magicMethods_accessModifier.SetOptions(
			"<none>",
			"private",
			"protected",
			"internal",
	...
	static Vector2 generalScrollPosition;
	static void GeneralSettings()
	{
		...
		generalScrollPosition = GUILayout.BeginScrollView(generalScrollPosition, noLayoutOptions);
...
#if UNITY_EDITOR_OSX
		EditorGUILayout.HelpBox("Hold the Cmd key pressed down while opening a file to temporarily invert the settings above.", MessageType.Info, true);
#else
		EditorGUILayout.HelpBox("Hold the Ctrl key pressed down while opening a file to temporarily invert the settings above.", MessageType.Info, true);
#endif
		EditorGUILayout.Space();
	...
	static void ViewSettings()
	{
		...
		GUILayout.Label("Miscelaneous", EditorStyles.boldLabel, noLayoutOptions);
		...
		GUILayout.Label("Line Spacing", EditorStyles.boldLabel, noLayoutOptions);
		labelWidth = 200f;
		Draw("Before Next Line", lineSpacing, 0, 8);
		//Draw("Between Word-Wrapped Lines", wrappedLinesSpacing, 0, 8);
		labelWidth = 250f;
		EditorGUILayout.Space();
		...
		GUILayout.Label("Code Folding", EditorStyles.boldLabel, noLayoutOptions);
		Draw2("C# Scripts", codeFoldingCode, codeFoldingCodeInspector);
		labelWidth = 250f;
		Draw(".  Auto-Fold Regions", autoFoldRegions, codeFoldingCode || codeFoldingCodeInspector);
		EditorGUILayout.Space();
		...
		EditorGUILayout.HelpBox("Inspecting values of properties requires executing their getters code.\n\n" +
			...
			"Alternatively disable inspecting values only on properties with side-effects by decorating them with:\n" +
...
Compile and reload assemblies with " + ctrlR + " or with a 'double-save'.",
		...
		Draw(isOSX ? "Swap Shift-Enter and Cmd-Enter" : "Swap Shift+Enter and Ctrl+Enter", swapShiftAndCtrlEnter);
		...
		Draw("Ignore punctuation marks", wordBreak_IgnorePunctuations);
		...
		labelWidth = 150f;
		Draw("Access modifier", magicMethods_accessModifier);
		labelWidth = 250f;
	...
	static string Draw(string label, StringOption option, bool enabled = true)
	{
#if UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_3_5
		...
		var index = ArrayUtility.FindIndex(option.options, x => x == option.Value);
		index = EditorGUILayout.Popup(label, index, option.options, noLayoutOptions);
		if (index >= 0 && index < option.options.Length)
			option.Value = option.options[index];
