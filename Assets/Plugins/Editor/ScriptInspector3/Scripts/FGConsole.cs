...
Click the button below to open the Console window...", MessageType.Info);
		...
		try
		{
...
#endif
			if (rc.xMax < position.width - 530f)
			{
			...
			if (rc.xMax < position.width - 415f)
			{
	...
	private class CallStackFramesEnumeratorBase
	{
		...
		public bool MoveNext()
		{
			while (++index < lines.Length)
			{
				...
				if (atAssetsIndex < 0)
					atAssetsIndex = textLine.IndexOf("(at Packages/");
				...
				else
				{
					...
					var functionNameStart = fileNameStartIndex;
					if (textLine[fileNameStartIndex] != '[')
					{
						fileNameStartIndex = textLine.IndexOf(" (at ", fileNameStartIndex + 1);
						if (fileNameStartIndex < 0)
							continue;
						fileNameStartIndex += " (at ".Length;
					}

					var fileNameEndIndex = textLine.LastIndexOf(':');
					...
					{
						lineEndIndex = textLine.IndexOf(")", lineStartIndex);
						if (lineEndIndex <= lineStartIndex)
							continue;
					}
					...
					{
						fileName = fileName.Substring(Application.dataPath.Length - "Assets".Length);
						current.guid = AssetDatabase.AssetPathToGUID(fileName);
					...
					else
					{
						...
						functionNameStart = lineStartIndex + 2;
	...
	private static void OpenLogEntry(EditorWindow console)
	{
		...
		if (listViewRow >= 0)
		{
			...
			if (gotIt)
			{
				...
				file = file.Replace('\\', '/');
				if (file.StartsWithIgnoreCase("assets/") || file.StartsWithIgnoreCase("packages/"))
				{
				...
				if (!string.IsNullOrEmpty(guid))
				{
					...
					else
					{
						...
						if (assetPath.StartsWithIgnoreCase("assets/") || file.StartsWithIgnoreCase("packages/"))
						{
	...
	private void DoPopupMenu(EditorWindow console)
	{
		...
		foreach (var stackFrame in new CallStackFrames(text))
		{
			if (!string.IsNullOrEmpty(stackFrame.guid))
			{
				...
				var functionName = stackFrame.functionName != null ? " - " + stackFrame.functionName.Replace(':', '.').Replace('/', '.') : "";
