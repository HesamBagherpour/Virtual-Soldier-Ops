...
public static class FGFindInFiles
{
	...
	public static SymbolDeclaration FindDeclarations(SymbolDefinition symbol)
	{
	...
	public static void RenameSymbol(SymbolDefinition symbol, string localAssetPath)
	{
		...
		if (symbol.kind == SymbolKind.Accessor || symbol.kind == SymbolKind.UsingAlias || symbol.kind == SymbolKind.TupleType)
			return;
	...
	static FindResultsWindow.ResultType ValidateResultAsReference(string assetGuid, TextPosition location, int length, ref SymbolDefinition referencedSymbol)
	{
		try
		{
			...
			{
				buffer.LoadImmediately();
				var newReferencedSymbol = referencedSymbol.Rebind();
				if (newReferencedSymbol != null)
				{
					referencedSymbol = newReferencedSymbol;
				}
				#if SI3_WARNINGS
				else
				{
					Debug.LogWarning("Couldn't rebind referenced symbol " + referencedSymbol.GetTooltipText());
				}
				#endif
			}
		
			var formattedLine = buffer.formattedLines[location.line];
			...
			if (isCsScript)
			{
				if (formattedLine.regionTree.kind > FGTextBuffer.RegionTree.Kind.LastActive)
				{
					if (isVarResult)
						return FindResultsWindow.ResultType.RemoveResult;
	...
	public static List<string> GetOrReadAllLines(string assetGuid)
	{
		var assetPath = AssetDatabase.GUIDToAssetPath(assetGuid);
		if (assetPath == "")
			assetPath = assetGuid;
	...
	public static List<string> GetOrReadAllLinesForPath(string assetPath)
	{
		if (string.IsNullOrEmpty(assetPath))
			return null;
	...
	internal static IEnumerable<TextPosition> FindAll(List<string> lines, FindResultsWindow.SearchOptions search)
	{
	...
	//static Unity.Profiling.ProfilerMarker containsWordsSequence = new Unity.Profiling.ProfilerMarker(
	//	Unity.Profiling.ProfilerCategory.Scripts,
	//	"ContainsWordSequence",
	//	Unity.Profiling.LowLevel.MarkerFlags.AvailabilityEditor);

	static char[] readCharsBuffer = new char[1024];
	...
	public static bool ContainsWordsSequence(string assetPath, params string[] words)
	{
		...
		try
		{
			//using (containsWordsSequence.Auto())
			using (var sr = new StreamReader(assetPath))
			{
				var findingDelegate = words.Length == 2 && words[1] == "(";
				...
				while (true)
				{
					if (i == numRead)
					{
						numRead = sr.Read(readCharsBuffer, 0, readCharsBuffer.Length);
						if (numRead == 0)
							return word == null;
	...
	public static void FindAllAssemblyScripts(AssemblyDefinition assembly)
	{
		...
		if (assembly.scriptAssembly != null)
		{
			var sourceFiles = assembly.scriptAssembly.sourceFiles;
			count = sourceFiles.Length;

			if (assets == null)
				assets = new List<string>(count);
			else if (assets.Capacity < count)
				assets.Capacity = count;
		...
		for (var i = count; i --> 0; )
		{
			...
			var assemblyFromPath = AssemblyDefinition.FromAssetPath(lowerPath);
