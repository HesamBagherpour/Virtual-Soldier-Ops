...
//#endif
	...
	public float GetLineOffset(int index)
	{
		...
		if (index >= yLineOffsets.Count)
		{
			...
			if (wordWrapping)
			{
				...
				var softLineBreaks = last < numSoftLineBreaks ?
					_softLineBreaks[last] ?? NO_SOFT_LINE_BREAKS : NO_SOFT_LINE_BREAKS;
				
				while (index > last)
				{
					var overlappingSpan = GetCharCollapsedSpan(last + 1, -1);
					while (overlappingSpan != null)
					{
						...
						overlappingSpan = GetCharCollapsedSpan(last + 1, -1);
					}

					y += softLineBreaks.Count + 1;
					yLineOffsets.Add(y);
					
					++last;
					softLineBreaks = last < numSoftLineBreaks ?
						_softLineBreaks[last] ?? NO_SOFT_LINE_BREAKS : NO_SOFT_LINE_BREAKS;
				}
			}
			else
			{
				while (index > last)
				{
					//while (IsLineBeginningCollapsed(last + 1))
					//{
					//	yLineOffsets.Add(y);
					//	++last;
					//}
	...
	private void InvalidateSoftLineBreaks(int fromLine, int toLine)
	{
		if (_softLineBreaks != null)
		{
			if (_softLineBreaks.Count < toLine)
				toLine = _softLineBreaks.Count;
...
//				parentWindow.Focus();
	...
	private static List<KeyValuePair<TextInterval, FGTextBuffer.CollapsibleTextSpan>> tempCollapsibleTextSpans = new List<KeyValuePair<TextInterval, FGTextBuffer.CollapsibleTextSpan>>();
	...
	private static List<KeyValuePair<TextInterval, FGTextBuffer.CollapsibleTextSpan>> tempOverlappingSpans = new List<KeyValuePair<TextInterval, FGTextBuffer.CollapsibleTextSpan>>();
	...
	private FGTextBuffer.CollapsibleTextSpan GetCollapsedSpanForTextSpan(TextSpan span)
	{
		...
		foreach (var spanKV in tempOverlappingSpans)
			if (spanKV.Value.IsCollapsed)
				return spanKV.Value;
	...
	public void UnfoldTextRange(int fromLine, int fromCharIndex, int toLine, int toCharIndex)
	{
		...
		{
			var span = tempOverlappingSpans[i].Value;
			if (!span.IsCollapsed)
				continue;
	...
	private FGTextBuffer.CollapsibleTextSpan GetCharCollapsedSpan(int line, int index)
	{
		...
		for (var i = 0; i < numSpans; ++i)
		{
			var spanKV = tempOverlappingSpans[i];
			if (spanKV.Value.IsCollapsed)
				return spanKV.Value;
		}
	...
	private List<int> GetSoftLineBreaks(int line)
	{
		...
		if (yLineOffsets != null && line + 1 < yLineOffsets.Count)
		{
			...
			while (IsLineBeginningCollapsed(line + 1))
				++line;
	...
	internal Vector2 BeginScrollView(Rect position, Vector2 scrollPosition, Rect viewRect)
	{
#if UNITY_5_3 || UNITY_5_4_OR_NEWER
	...
	public void EndScrollView(bool handleScrollWheel = true)
	{
#if UNITY_5_3 || UNITY_5_4_OR_NEWER
		GUI.EndClip();
	...
	static List<KeyValuePair<TextInterval, FGTextBuffer.CollapsibleTextSpan>> collapsibleTextSpansInView = new List<KeyValuePair<TextInterval, FGTextBuffer.CollapsibleTextSpan>>();
	...
	beginScrollViewAgain:
		...
		if (enableCodeFolding && Event.current.type == EventType.MouseDown && Event.current.button == 0)
		{
			...
			if (Event.current.mousePosition.x >= rect.x && Event.current.mousePosition.x < rect.xMax)
			{
				...
				for (var i = 0; i < numSpans; ++i)
				{
					var span = collapsibleTextSpansInView[i].Value;
					...
					if (rect.Contains(Event.current.mousePosition))
					{
						...
						OnFoldText(span.span.Start, span.span.End);
						goto endScrollViewAndExit;
					}
					if (span.IsCollapsed)
		...
		if (enableCodeFolding)
		{
			...
			if (eventType == EventType.MouseMove || findHighlightedCollapsibleTextSpan && eventType == EventType.Repaint)
			{
				...
				else
				{
					...
					if (oldHighlighted != highlightedCollapsibleTextSpan)
					{
						needsRepaint = true;
					}
				}
		...
		if (enableCodeFolding)
		{
			if (Event.current.type == EventType.Repaint)
			{
				...
				for (var i = collapsibleTextSpansInView.Count; i -- > 0; )
				{
					...
					rect.y = GetLineOffset(span.span.Start.line) + pointSize;
					rect.yMax = GetLineOffset(span.span.End.line + 1);
					...
					if (!span.IsCollapsed && Event.current.type == EventType.Repaint)
					{
						var fillRect = rect;
				...
				{
					var span = collapsibleTextSpansInView[spanIndex].Value;
					while (i > span.span.Start.line)
					{
						...
						span = collapsibleTextSpansInView[spanIndex].Value;
					}
			...
			if (trackChanges)
			{
				...
				for (int i = fromLine; i <= toLine; ++i)
				{
					...
					//if (savedVersion > 0 || version > 0)
					{
						...
						bool isCollapsedLine = IsLineBeginningCollapsed(i);
						if (isCollapsedLine)
						{
						...
						if (lastChange == 0)
							FillRect(lastRect, styles.trackChangesAfterSaveColor);
						else if (lastChange == 1)
							FillRect(lastRect, styles.trackChangesBeforeSaveColor);
						else if (lastChange == -1)
							FillRect(lastRect, styles.trackChangesRevertedColor);
					}
				}
			}
		}
...
//					return;
	...
	private FGTextBuffer.CollapsibleTextSpan FindCallapsibleSpansInSelection(FindFoldingSpansOptions options)
	{
		tempOverlappingSpans.Clear();
		...
		var result = tempOverlappingSpans[numSpans - 1].Value;
		...
		if ((options & FindFoldingSpansOptions.OnlyFullyContained) != 0)
		{
			for (var i = numSpans; i --> 0; )
			{
				var span = tempOverlappingSpans[i].Value.span;
	...
	private void ToggleFoldingAtCaret()
	{
		...
		var lastSpan = FindCallapsibleSpansInSelection(FindFoldingSpansOptions.OnlyFullyContained);
		if (lastSpan == null)
			return;
	...
	private void FoldAtCaret()
	{
		...
		var lastSpan = FindCallapsibleSpansInSelection(FindFoldingSpansOptions.OnlyFullyContained | FindFoldingSpansOptions.OnlyUnfolded);
		if (lastSpan == null)
			return;
	...
	private void UnfoldAtCaret()
	{
		...
		var lastSpan = FindCallapsibleSpansInSelection(FindFoldingSpansOptions.OnlyFullyContained | FindFoldingSpansOptions.OnlyFolded);
		if (lastSpan == null)
			return;
	...
	private void FoldText(TextPosition from, TextPosition to)
	{
		...
		if (addedSpan != null && addedSpan != span)
		{
	...
	private void ToggleFolding(FGTextBuffer.CollapsibleTextSpan span)
	{
		if (span.IsCollapsed)
		{
			...
			InvalidateSoftLineBreaks(span.span.Start.line, span.span.End.line);
			TextBuffer.RepaintAllEditors();
		}
		else
		{
			span.IsCollapsed = true;
				...
				case KeyCode.UpArrow:
					...
					else if (modifiers == EventModifiers.Alt)
					{
						...
						if (moveLinesFrom > 0)
						{
							...
							{
								var span = tempCollapsibleTextSpans[i].Value;
