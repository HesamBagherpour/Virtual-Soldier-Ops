...
namespace ScriptInspector
{
	...
	public class TextIntervalTree<TypeValue>
    {
        ...
        public void GetIntervalsOverlappingWith(TextInterval toFind, List<KeyValuePair<TextInterval, TypeValue>> list)
        {
            if (this.Root != null)
            {
        ...
        private bool TryGetIntervalImpl(IntervalNode subtree, TextInterval data, out TypeValue value)
        {
            if (subtree != null)
            {
                ...
                else
                {
                    ...
                    {
	                    int kthIndex = subtree.Range.BinarySearch(new KeyValuePair<TextPosition, TypeValue>(data.End, default(TypeValue)), this.keyvalueComparer);
                        if (kthIndex >= 0)
                        {
        ...
        private class IntervalNode
        {
	        ...
	        public static List<KeyValuePair<TextInterval, TypeValue>> GetIntervalsStartingAt(IntervalNode subtree, TextPosition start)
            {
                if (subtree != null)
                {
                    ...
                    else
                    {
                        ...
                        result.Add(new KeyValuePair<TextInterval, TypeValue>(subtree.Interval, subtree.Value));
                        return result;
                    }
                }
                else
            ...
            public void GetIntervalsOverlappingWith(TextInterval toFind, List<KeyValuePair<TextInterval, TypeValue>> list)
            {
                if (toFind.End.CompareTo(this.Interval.Start) <= 0)
                {
                    ...
                    if (this.Left != null)
                    {
                        this.Left.GetIntervalsOverlappingWith(toFind, list);
                    }
                }
                else if (toFind.Start.CompareTo(this.Max) >= 0)
                {
                    ...
                    if (this.Right != null)
                    {
                        this.Right.GetIntervalsOverlappingWith(toFind, list);
                    }
                }
                else
                {
                    ...
                    {
                        list.Add(new KeyValuePair<TextInterval, TypeValue>(this.Interval, this.Value));
