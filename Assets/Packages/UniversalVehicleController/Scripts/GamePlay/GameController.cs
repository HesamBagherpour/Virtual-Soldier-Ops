...
namespace PG
{
    ...
    public class GameController :Singleton<GameController>
    {
        ...
        public bool gameIsRunning { set; get; } = false;


        List<VehicleController> AllVehicles = new List<VehicleController>();
        Vector3 playerInitPos;
        Quaternion playerInitRot;
        GameObject currentAICars;
        ...
        void Start ()
        {
            ...
            currentAICars = Instantiate(AICars);
            playerInitPos = AllCars[0].transform.position;
            playerInitRot = AllCars[0].transform.rotation;
        ...
        InitializePlayer UpdateSelectedCar (InitializePlayer player, CarController car)
        {
            if (!player)
            {
                ...
                player = GameObject.Instantiate (playerPrefab,car.transform);
            }
        ...
        public void SetPlayerTypeToDriver() => playerRole.SetPlayerRole(PlayerRole.Role.Driver);
        public void SetPlayerTypeToBodyguard() => playerRole.SetPlayerRole(PlayerRole.Role.Bodyguard);

        public void PauseGame()
        {
            foreach (Transform aiCar in currentAICars.transform)
            {
                aiCar.GetComponent<RaceAIControl>().Proceed = false;
            }
            gameIsRunning = false;
        }
        ...
        public void ResumeGame()
        {
            foreach (Transform aiCar in currentAICars.transform)
            {
                aiCar.GetComponent<RaceAIControl>().Proceed = true;
            }
            gameIsRunning = true;
        }
        ...
        private IEnumerator ResetGame_CO()
        {
            Destroy(currentAICars);
            currentAICars = Instantiate(AICars);
            Rigidbody carRigid = AllCars[0].GetComponent<Rigidbody>();
            carRigid.isKinematic = true;
            AllCars[0].transform.SetPositionAndRotation(playerInitPos, playerInitRot);
