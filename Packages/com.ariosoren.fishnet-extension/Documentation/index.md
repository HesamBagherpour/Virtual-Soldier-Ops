...
Fish-Networking یک فریم‌ورک قدرتمند و انعطاف‌پذیر برای پیاده‌سازی قابلیت‌های چندنفره در بازی‌های یونیتی است. این فریم‌ورک امکانات گسترده‌ای برای مدیریت ارتباطات سرور و کلاینت، همگام‌سازی داده‌ها، و اجرای کدهای شبکه‌ای فراهم می‌کند.
...
در معماری شبکه‌ای، سه نقش اصلی وجود دارد:

- **سرور (Server)**: مسئول مدیریت وضعیت بازی، اعتبارسنجی اقدامات بازیکنان و هماهنگ‌سازی اطلاعات بین کلاینت‌ها است. سرور به عنوان منبع اصلی حقیقت در بازی عمل می‌کند.
...
- نکته: - آبجکت های که ownerId آنها برابر با -1 است هیچ مالکی ندارند.
	- آبجکت هایی که در Scene از ابتدا وجود دارند Scene Object گفته می شود.
...
- **متدها**:
  ...
  - متدهای RPC باید با پسوند `Rpc` **ختم** شوند و با توجه به نوعشان نام‌گذاری شوند. مثال:
	...
	- `kickReasonTargetRpc()`
- نکته: بهتر است تا حد امکان، استفاده از متد های مولتی تارکت را کم کنید و از چند پیاده سازی جداگانه استفاده نمایید.
...
- **کلاس‌ها**:
 ...
 - تمام متد ها و پراپرتی ها و متغییر های پابلیک در کلاس باید رفتار مشخص شده در شبکه داشته باشند.
...
- تمامی کلاس‌هایی که با شبکه تعامل دارند باید از `NetworkBehaviour` مشتق شوند.
...
- تمامی ورودی‌های دریافت‌شده از کلاینت‌ها باید در سمت سرور اعتبارسنجی شوند.
...
## 5. کار با NetworkBehaviour و NetworkObject
...
**توجه**: اطمینان حاصل کنید که عملیات اسپاون و دی‌اسپاون فقط در سمت سرور انجام می‌شود و تمامی کلاینت‌ها به درستی از این تغییرات مطلع می‌شوند.
...
یک `NetworkObject` تو در تو ممکن است از والد خود جدا شود، اما حتی در صورت جدا شدن از والد، باز هم با دی‌اسپاون شدن والد، آن نیز دی‌اسپاون می‌شود. این اتفاق به این دلیل می‌افتد که والد یک مرجع کش شده از رفتارها و اشیاء تو در تو خود دارد و این مراجع نمی‌توانند بدون هزینه عملکردی غیرقابل قبول به صورت خودکار به‌روزرسانی شوند. با این حال، این یک ویژگی است که قصد داریم در آینده بررسی کنیم تا به توسعه‌دهندگان اجازه دهیم به صورت دستی مشخص کنند که یک شیء تو در تو از والد خود جدا شده است.
